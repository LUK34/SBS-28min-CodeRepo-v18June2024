...
public class UserJpaResourceController 
{
	...
	public UserJpaResourceController( UsersRepo urepo, PostRepo prepo)
	{
		//this.ud=ud;
		this.urepo=urepo;
		this.prepo=prepo;
	...
	public EntityModel<Users> retrieveUser(@PathVariable Long id)
	{
	//	Users user = ud.findOne(id);urepo.findById(id);
	...
	public ResponseEntity<Object> postPostsForUser(@PathVariable Long id, @Valid @RequestBody Post posty)
	{
		Optional<Users> user = urepo.findById(id);
		if(user.isEmpty())
		{
		...
		posty.setUser(user.get());
		
		Post savedPost = prepo.save(posty);
	...
	public ResponseEntity<Users> createUser(@Valid @RequestBody Users user)
	{
		...
		URI location=ServletUriComponentsBuilder
								...
								.toUri(); 
		/*
		 what we will do is we will take /user and append {id}  and we want to replace the value of id with user.getID.
		 The entire thimg is later convert to URI
		 */
		return ResponseEntity.created(location).build();
	}
